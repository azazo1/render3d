...
#[inline]
#[must_use]
const fn rgbf(r: f32, g: f32, b: f32) -> u32 {
...
impl FpsCounter {
    ...
    fn new(count_window: Duration) -> Self {
        Self {
            count_window,
            tick_history: [].into(),
        }
...
impl Vec3 {
    ...
    #[inline]
    #[must_use]
    const fn dot(self, rhs: Self) -> f32 {
...
impl Light {
    fn new(pos: Vec3, strength: f32) -> Self {
        Self { pos, strength }
    }
}
...
/// 渲染一个 3D 场景, 地面为 z = 0.
#[derive(Debug)]
struct Render3D<R: Rng + Send + Sync + Clone> {
    width: usize,
    height: usize,
    buffer: Vec<u32>,
...
impl<R: Rng + Send + Sync + Clone> Render3D<R> {
    /// 相机移动速度 (米/s).
    ...
    fn put_sphere(&mut self, sphere: Sphere) -> &mut Self {
        self.spheres.push(sphere);
        self
    }

    fn put_light(&mut self, light: Light) -> &mut Self {
    ...
    fn handle_keys(&mut self, window: &Window) {
        ...
        if window.is_key_down(minifb::Key::J) {
            delta_angle = delta_angle - Vec3::Y;
        }
        if window.is_key_down(minifb::Key::K) {
            delta_angle = delta_angle + Vec3::Y;
        }
    ...
    fn render(&mut self, window: &Window) {
        ...
        self.buffer = (0..self.height * self.width)
            .into_par_iter()
            .chunks(1000)
            .map_init(
                || self.rng.clone(),
